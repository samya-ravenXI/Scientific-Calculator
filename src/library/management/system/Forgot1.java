...
public class Forgot1 extends JFrame implements ActionListener{
	...
	panel.setBorder(new TitledBorder(new LineBorder(new Color(139, 69, 19), 2), "Forgot-Password-Panel",
    ...
    public void actionPerformed(ActionEvent ae){
        try{
            conn con = new conn();
            if(ae.getSource() == b1){
                String sql = "select * from account_srf,student where account_srf.student_id=? and student.student_id=?";
		...
		while (rs.next()) {
            ...
            if(ae.getSource() == b2){
                String sql = "select * from account_srf where sec_ans=?";
		PreparedStatement st = con.c.prepareStatement(sql);
		...
		new Login_user1().setVisible(true);
