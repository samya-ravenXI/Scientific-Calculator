...
public class RequestBook extends JFrame implements ActionListener{
	...
	JLabel l6 = new JLabel("Request_id");
	...
	panel.setBorder(new TitledBorder(new LineBorder(new Color(255, 0, 0), 2), "Request Books", TitledBorder.LEADING,
			TitledBorder.TOP, null, new Color(255, 0, 0)));
	panel.setBounds(10, 29, 398, 330);
    ...
    public void actionPerformed(ActionEvent ae){
        try{
            conn con = new conn();
            if(ae.getSource() == b1){
                String sql = "insert into requestbook(book_id, student_id, name, author, publisher, edition, price, pages) values(?, ?, ?, ?, ?, ?, ?, ?)";
		PreparedStatement st = con.c.prepareStatement(sql);
                // st.setInt(1, Integer.parseInt(textField.getText()));
		st.setString(1, t1.getText());
                st.setString(2, Login_user1.s);
